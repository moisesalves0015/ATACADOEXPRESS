...
const canTransitionTo = (current: OrderStatus, next: OrderStatus): boolean => {
  if (current === next) return false; // RISCO-09: no-op should not trigger modal
  ...
  if (current === 'entregue') return false; // Final state
          ...
          <div className="bg-white rounded-t-3xl sm:rounded-2xl w-full max-w-4xl shadow-2xl flex flex-col max-h-[95vh] animate-in slide-in-from-bottom duration-300 overflow-hidden">
            <div className="p-6 border-b border-gray-50 flex items-center justify-between bg-white z-10 shrink-0">
              <div className="flex items-center gap-4">
                <h2 className="text-xl font-black text-gray-900 tracking-tight">Pedido #{selectedOrder.id.slice(-6).toUpperCase()}</h2>
              ...
              <div className="flex items-center gap-2">
                <button 
                  ...
                  className="hidden sm:flex items-center gap-2 px-4 py-2 bg-gray-900 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-gray-800 transition-all shadow-lg shadow-gray-200"
                >
                  <Download className="w-4 h-4" /> PDF
                </button>
                <button onClick={() => setSelectedOrder(null)} className="p-2 text-gray-400 hover:text-gray-900 rounded-full hover:bg-gray-100 transition-all">
                  <X className="w-7 h-7" />
                </button>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    <div>
                      ...
                      <div className="space-y-1">
                        <p className="font-black text-gray-900 text-xl leading-tight">{selectedOrder.clientName}</p>
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{selectedOrder.clientEmail || 'Sem email'}</p>
                        {selectedOrder.clientPhone && (
                          <div className="mt-3">
                             <a 
                               href={`https://wa.me/55${selectedOrder.clientPhone.replace(/\D/g, '')}`} 
                               target="_blank" 
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        <div>
                          <p className="text-xs font-black text-gray-900 uppercase tracking-widest">{format(new Date(selectedOrder.orderDate), "dd 'de' MMMM", { locale: ptBR })}</p>
                          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{format(new Date(selectedOrder.orderDate), "HH:mm")}</p>
                        </div>
                        {selectedOrder.sellerId && (
                          <div className="pt-3 border-t border-gray-50">
                            ...
                            <p className="text-sm font-black text-emerald-600">{selectedOrder.sellerName}</p>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <div className={cn(
                    ...
                    selectedOrder.orderOrigin === 'admin' ? 'bg-purple-50/50 border-purple-100' : 'bg-blue-50/50 border-blue-100'
                  )}>
                    <div className={cn("p-2.5 rounded-xl shadow-lg", selectedOrder.orderOrigin === 'admin' ? "bg-purple-600 shadow-purple-100" : "bg-blue-600 shadow-blue-100")}>
                      {selectedOrder.orderOrigin === 'admin' ? <ShieldCheck className="w-5 h-5 text-white" /> : <Globe className="w-5 h-5 text-white" />}
                    </div>
                    <div>
                      <p className={cn('text-[10px] font-black uppercase tracking-widest', selectedOrder.orderOrigin === 'admin' ? 'text-purple-700' : 'text-blue-700')}>
                        Origem: {selectedOrder.orderOrigin === 'admin' ? 'Registro Administrativo' : 'E-commerce (Cliente)'}
                      </p>
                      {selectedOrder.orderOrigin === 'admin' && selectedOrder.registeredByAdminName && (
                        <p className="text-xs text-purple-600 mt-1">Registrado por <strong>{selectedOrder.registeredByAdminName}</strong></p>
                      )}
                    </div>
                  </div>
                          ...
                          <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-gray-50">
                              {selectedOrder.items.map((item, idx) => {
                                const sConf = statusConfig[item.status || selectedOrder.status || 'aguardando_pagamento'];
                                  ...
                                  <tr key={idx} className="hover:bg-gray-50/30 transition-all">
                                    ...
                                    <td className="px-5 py-4">
                                      <div className="flex justify-center">
                                        <select
                                          value={item.status || selectedOrder.status || 'aguardando_pagamento'}
                                          onChange={(e) => handleUpdateItemStatus(selectedOrder.id, idx, e.target.value as OrderStatus, (item.status || selectedOrder.status || 'aguardando_pagamento') as OrderStatus)}
                                          className={cn(
                        ...
                        <div className="p-6 bg-gray-50 flex flex-col items-end gap-3 border-t border-gray-100">
                           <div className="w-full max-w-[320px] space-y-2">
                             <div className="flex justify-between items-center text-[10px] font-black text-blue-600 uppercase tracking-widest bg-blue-100/50 px-3 py-2 rounded-xl border border-blue-100/50">
                               ...
                               <span className="text-sm">{fmt(selectedOrder.totalReady || 0)}</span>
                             </div>
                             ...
                             {(selectedOrder.totalPending || 0) > 0 && (
                               <div className="flex justify-between items-center text-[10px] font-black text-orange-600 uppercase tracking-widest bg-orange-100/50 px-3 py-2 rounded-xl border border-orange-100/50">
                                 ...
                                 <span className="text-sm">{fmt(selectedOrder.totalPending || 0)}</span>
                               </div>
                             )}
                             ...
                             <div className="flex justify-between items-center w-full pt-4 border-t border-gray-200">
                               ...
                               <span className="text-2xl font-black text-brand-pink">{fmt(selectedOrder.totalValue)}</span>
                             </div>
                           </div>
                        </div>
                    </div>
                  ...
                  <div className="relative pl-6 space-y-8 before:absolute before:left-[27px] before:top-2 before:bottom-2 before:w-0.5 before:bg-gray-100">
                    ...
                    <div className="relative">
                      <div className="absolute -left-[32px] top-1 w-8 h-8 rounded-xl bg-white border border-gray-100 z-10 flex items-center justify-center shadow-sm">
                        <Plus className="w-4 h-4 text-gray-300" strokeWidth={3} />
                      </div>
                      <div className="space-y-1">
                        <p className="text-[11px] font-black text-gray-900 uppercase tracking-tight">Pedido Criado</p>
                        <p className="text-[10px] font-bold text-gray-400 uppercase">{format(new Date(selectedOrder.orderDate), "dd MMM · HH:mm", { locale: ptBR })}</p>
                      </div>
                    </div>
                      ...
                      <div key={i} className="relative">
                        ...
                        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
                          <div className="flex justify-between items-start mb-2">
                             <p className={cn("text-[10px] font-black uppercase tracking-widest", statusConfig[h.status]?.color)}>
                               {statusConfig[h.status]?.label}
                             </p>
                             {h.isInternal && (
                               <span className="flex items-center gap-1 text-[8px] bg-amber-50 text-amber-600 px-2 py-0.5 rounded-lg border border-amber-100 font-black uppercase tracking-widest">
                                 <Lock className="w-2.5 h-2.5" /> Interno
                               </span>
                             )}
                          </div>
                          <p className="text-[9px] font-bold text-gray-300 uppercase mb-3">
                             {format(new Date(h.updatedAt), "dd MMM · HH:mm", { locale: ptBR })} · <strong>{h.updatedBy}</strong>
                          </p>
                          {h.comment && (
                            <div className="text-xs font-medium text-gray-600 bg-gray-50 p-3 rounded-xl border border-gray-100/50">
                               {h.comment}
                            </div>
                          )}
                        </div>
            ...
            <div className="p-6 bg-gray-50 border-t border-gray-100 sm:hidden pb-10">
               <button 
                 onClick={() => generateOrderPDF(selectedOrder)}
                 className="w-full py-4 bg-gray-900 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-xl shadow-gray-200 flex items-center justify-center gap-3"
               >
